...
class FlipkartScraper:
    ...
    def get_top_reviews(self, product_url, count=2):
        ...
        reviews = []
        cards = soup.select("div.mlW33x")  # REVIEW CONTAINER

        for card in cards:
            try:
                rating = card.select_one("div.MKiFS6.ojKpP6").get_text(strip=True)[0]
                title = card.select_one("p.qW2QI1").get_text(strip=True)
                text = card.select_one("div.G4PxIA > div > div").get_text(strip=True)

                reviews.append(f"{rating}★ {title}: {text}")
            except:
                continue
